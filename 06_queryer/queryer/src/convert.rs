...
// 因为rust trait的孤儿规则，我们如果要对已有的类型实现已有的trait，需要简单包装一下
...
/// 把SqlParser解析出来的Statement转换成我们需要的结构
impl<'a> TryFrom<&'a Statement> for Sql<'a> {
    type Error = anyhow::Error;

    fn try_from(sql: &'a Statement) -> Result<Self, Self::Error>{
        match sql {
            // 目前我们只关心query (selection ... from ... where ...)
            Statement::Query(q) => {
...
/// 把SqlParser的SelectItem转换成DataFrame的Expr
...
/// 把SqlParser的order by expr 转换成( 列名， 排序方法)
...
/// 把Sqlparser的Limit expr 转换成unsize
...
/// 把SqlParser的value转换成DataFrame支持的LiteralValue
